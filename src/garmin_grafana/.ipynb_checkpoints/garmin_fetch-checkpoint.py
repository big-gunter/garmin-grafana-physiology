...
def _range_utc(start_date: str, days: int) -> tuple[str, str]:
    # inclusive start at 00:00Z, exclusive end at 00:00Z next day
...
def _query_scalar_influx_v1(q: str) -> float | None:
    try:
        ...
        if not pts:
            return None
        # single column usually named 'percentile' or 'max' or 'median'
        row = pts[0]
        # try common keys
...
def _percentile_activity_maxhr_42d(asof_date: str) -> float | None:
    # 42d window ending at asof_date+1 (includes asof_date)
...
def _percentile_peak_1min_hr_42d(asof_date: str) -> float | None:
    """
    Approximation of "peak 1-minute HR" using HeartRateIntraday grouped into 1m means,
    then taking the maximum of those means across the window, then p90 across days is expensive.
    We instead compute p90 across all 1-minute bins in the 42d window (good practical proxy).
    """
    start = (_dt_utc(asof_date) - timedelta(days=41)).strftime("%Y-%m-%d")
    start_iso, end_iso = _range_utc(start, 42)

    # Subquery: 1-minute mean bins across the whole 42d range; percentile over those bins.
    # Note: InfluxQL subqueries are supported in InfluxDB 1.x.
    q = (
    ...
    # Fallback if subquery isn’t supported for any reason:
    q2 = (
...
def _hrr_zones(rhr: float, hrmax: float) -> dict[str, float]:
    """
    5-zone Karvonen (%HRR) defaults (common):
      Z1 50–60
      Z2 60–70
      Z3 70–80
      Z4 80–90
      Z5 90–100
    Stored as bpm boundaries.
    """
    hrr = hrmax - rhr
    ...
    return {
        "HRR": hrr,
        "Z1_Low": bpm(0.50), "Z1_High": bpm(0.60),
        "Z2_Low": bpm(0.60), "Z2_High": bpm(0.70),
        "Z3_Low": bpm(0.70), "Z3_High": bpm(0.80),
        "Z4_Low": bpm(0.80), "Z4_High": bpm(0.90),
        "Z5_Low": bpm(0.90), "Z5_High": bpm(1.00),
    }

def compute_and_write_physiology(asof_date: str) -> None:
    """
    Writes one daily point to measurement PhysiologyDaily for the given date (YYYY-MM-DD).
    """
    if INFLUXDB_VERSION != "1":
    ...
    if rhr7 is None or (p95_act is None and p90_1m is None):
        logging.info(f"PhysiologyDaily: insufficient data for {asof_date} (rhr7={rhr7}, p95_act={p95_act}, p90_1m={p90_1m})")
        return
