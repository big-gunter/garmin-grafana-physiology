"""
MCP server: read-only InfluxDB (InfluxQL) for Garmin stats.

Run (stdio, for Claude Desktop):
  INFLUXDB_HOST=localhost INFLUXDB_PORT=8086 ... python -m garmin_mcp.influx_read

Docker Compose: expose Influx to the host (127.0.0.1:8086) when MCP runs on your laptop.
"""
...
from ._env import docs_dir, load_influx_settings
...
def influxql_query(query: str, max_rows: int = 500) -> str:
    ...
    out = df.head(mr)
    return json.dumps(
        {"rows": out.to_dict(orient="records"), "row_count": int(len(out)), "truncated": len(df) > mr},
        default=str,
    )
